  ...
  connect () {
    return new Promise((resolve, reject) => {
      ...
      client.on('data', () => {
        ...
        const soapBody = '<u:X-pairingRequest xmlns:u="urn:schemas-upnp-org:service:X-CTC_RemotePairing:1"><pairingDeviceID>' + this.config.terminalId + '</pairingDeviceID><friendlyName>iobroker</friendlyName><userID>' + this.config.userId + '</userID></u:X-pairingRequest>'
    ...
    expressApp.use('*', (req, res, next) => {
      ...
      req.on('end', () => {
        if (body.indexOf('X-pairingCheck:') >= 0) {
          ...
          this.verificationCode = createHash('md5').update(this.pairingCode + this.config.terminalId + this.config.userId).digest('hex').toUpperCase()
