...
export default class MagentaTvRemote {
  ...
  connect () {
    return new Promise((resolve, reject) => {
      ...
      client.on('data', () => {
        ...
        const soapBody = '<u:X-pairingRequest xmlns:u="urn:schemas-upnp-org:service:X-CTC_RemotePairing:1"><pairingDeviceID>' + this.config.terminalId + '</pairingDeviceID><friendlyName>iobroker</friendlyName><userID>' + this.config.userHash + '</userID></u:X-pairingRequest>'
  ...
  sendKeyCode (keyCode) {
    const soapBody = '<u:X_CTC_RemoteKey xmlns:u="urn:schemas-upnp-org:service:X-CTC_RemoteControl:1"><InstanceID>0</InstanceID><KeyCode>keyCode=' + keyCode + '^' + this.config.terminalId + ':' + this.verificationCode + '^userID:' + this.config.userHash + '</KeyCode></u:X_CTC_RemoteKey>'
    const soapXml = '<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/" s:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/"><s:Body>' + soapBody + '</s:Body></s:Envelope>'
